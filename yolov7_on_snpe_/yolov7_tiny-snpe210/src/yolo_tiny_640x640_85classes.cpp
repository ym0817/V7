...
std::vector<std::vector<float>> sigmoid(std::vector<std::vector<float>> in){
    ...
    for (size_t i = 0; i < 3; i++) {
        int height = size[i];
        int width = size[i];
        for (int j = 0; j < height; j++) {      // 20/80/40
            for (int k = 0; k < width; k++) {   // 20/80/40
                int anchorIdx = 0;
                for (int l = 0; l < 3; l++) {   // 3
...
std::unique_ptr<zdl::SNPE::SNPE> initializeSNPE(zdl::DlSystem::Runtime_t runtime) {
    ...
    outputLayers.append("Conv_149");
    outputLayers.append("Conv_164");
    std::unique_ptr<zdl::DlContainer::IDlContainer> container;
    container = zdl::DlContainer::IDlContainer::open("/home/ymm/Workspace/Git/yolov7_on_snpe/yolov7_tiny-snpe210/models/yolov7-tiny-sim-1_168.dlc");   
...
int main() {
    ...
    //load_pic
    std::string image_path = "/home/ymm/Workspace/Git/yolov7_on_snpe/yolov7_tiny-snpe210/bus.jpg";
    ...
    cv::imwrite("picture_detection4.jpg", image_new);
